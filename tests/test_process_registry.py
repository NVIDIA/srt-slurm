...
def test_profile_wrapper_uses_task_only_step_signal():
    ...
    with patch("srtctl.core.processes.shutil.which", return_value="/bin/scancel"), patch(
        "srtctl.core.processes.subprocess.run", return_value=MagicMock(returncode=0)
    ) as run:
...
def test_profile_shutdown_metadata_survives_registry_rename():
    ...
    with patch("srtctl.core.processes.signal_step", return_value=True) as signal, patch(
        "srtctl.core.processes.list_step_ids", return_value={"profiled": "123.4"}
    ):
        registry.cleanup()
