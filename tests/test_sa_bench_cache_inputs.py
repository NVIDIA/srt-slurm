...
from srtctl.benchmarks.base import SCRIPTS_DIR, PrewarmPlan
from srtctl.benchmarks.sa_bench import DATASET_CACHE_MOUNT
from srtctl.benchmarks.sa_bench_cache import (
    PREWARM_ENV_VAR,
    parse_concurrencies,
    prompt_counts,
)
...
def _plan(config: SrtConfig, **options):
    """Plan through the runner hook the CLI uses, not the SA-Bench planner directly."""
    return get_runner(config.benchmark.type).plan_prewarm(config, **options)
...
class TestPrewarmHook:
    """cache-inputs asks the runner, so nothing outside SA-Bench knows about it."""

    def test_sa_bench_plans_through_the_runner_hook(self, tmp_path):
        plan = _plan(_config(tmp_path))

        assert isinstance(plan, PrewarmPlan)
        assert plan.title
        assert plan.summary_rows

    def test_benchmarks_without_prewarming_return_no_plan(self, tmp_path):
        """A runner that cannot pre-generate says so instead of raising."""
        assert _plan(_config(tmp_path, type="sglang-bench")) is None

    def test_summary_names_every_dataset_the_plan_builds(self, tmp_path):
        rows = dict(_plan(_config(tmp_path)).summary_rows)

        assert rows["Datasets (4)"] == "n=64, n=512, n=128, n=1024"
        assert rows["ISL / OSL"] == "8192 / 1024"
...
class TestPlan:
    ...
    def test_hf_models_are_not_mounted(self, tmp_path):
        ...
        plan = _plan(_config(tmp_path, model_path="hf:facebook/opt-125m"))
    ...
    def test_srun_uses_recipe_slurm_settings(self, tmp_path, monkeypatch):
        ...
        srun = _plan(_config(tmp_path)).srun_command()
    ...
    def test_worker_count_is_passed_to_the_generator(self, tmp_path):
        srun = _plan(_config(tmp_path), num_workers=32).srun_command()
