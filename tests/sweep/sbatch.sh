...
wait_for_http_ok "${PREFILL_NODE_A}" "${CTX0_PORT}" "/health" 300
wait_for_http_ok "${PREFILL_NODE_A}" "${CTX1_PORT}" "/health" 300
wait_for_http_ok "${PREFILL_NODE_B}" "${CTX2_PORT}" "/health" 300
wait_for_http_ok "${PREFILL_NODE_B}" "${CTX3_PORT}" "/health" 300
wait_for_http_ok "${DECODE_NODES[0]}" "${GEN_PORT}" "/health" 300
...
if ! wait_for_http_ok "${HEAD_NODE}" "${DISAGG_PORT}" "/health" 300; then
