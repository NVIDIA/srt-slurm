...
def _title_ok(title: str) -> bool:
    return (
        subprocess.run(["bash", str(SCRIPTS / "check_pr_title.sh"), title], capture_output=True, check=False).returncode
        == 0
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "fix: register git_http_version on ClusterConfig",
        "feat!: drop --bash",
        "refactor(core/roles)!: rename",
        "docs: describe only the 2.0 layout",
        'Revert "Add Slurm SGLang Dynamo sidecars (#354)"',
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "Fix SGLang sidecar launch and endpoint integration",
        "Fix: register git_http_version",  # capitalized type
        "feat:missing space",
        "feature: not a known type",
        "[2.0] The full 2.0 stack",
    ],
)
def test_free_form_titles_fail(title: str) -> None:
