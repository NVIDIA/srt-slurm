...
| `features/node-hooks.yaml` | `host_setup:` pre-run and post-run commands on each node's bare host, driven by `configs/node-hooks.sh`: an ordered list of arbitrary shell commands from `HOOK_PRE_<n>` / `HOOK_POST_<n>` recipe environment values, with per-command logging and a state snapshot |
