...
Vendored from NVIDIA/InferenceMAX runners/patch_vllm_simple_kv_offload.py at
commit 75908a871, the B200 MiniMax-M3 AgentX submission source.
"""
...
def patch_worker(worker_path: Path) -> bool:
    ...
    if source.count(OLD_SETUP) != 1 or source.count(OLD_LOOP) != 1:
        raise RuntimeError(f"unsupported vLLM SimpleCPUOffload worker at {worker_path}")
...
def main(argv: list[str]) -> int:
    ...
    try:
        worker_path = Path(argv[1]).resolve() if len(argv) == 2 else installed_worker_path()
