...
"""Direct vLLM serving without a separate frontend process."""
...
@register_frontend("vllm-direct")
class VLLMDirectFrontend:
    """Expose one aggregate ``vllm serve`` endpoint without a router."""
    ...
    def start_frontends(
        ...
        if config.backend.type != "vllm":
            raise ValueError(f"frontend.type: vllm-direct requires backend.type: vllm (got {config.backend.type!r})")
        if topology.uses_nginx or len(topology.frontend_nodes) != 1:
            raise ValueError(
                "frontend.type: vllm-direct binds vllm serve directly to the public port; "
                "set frontend.enable_multiple_frontends: false"
            )
        if config.resources.is_disaggregated or config.resources.num_agg < 1:
            raise ValueError("frontend.type: vllm-direct supports aggregate vLLM jobs only")

        logger.info(
            "frontend.type=vllm-direct: no separate frontend process; vllm serve owns port %d",
