...
Tachometer does not preserve Prometheus HELP/TYPE/unit metadata. Only known
counter families are differentiated; unknown families remain stored values.
...
def is_counter(name: str) -> bool:
    """Whether this known family is cumulative; never infer from monotonic data."""
    name = _canonical(name)
    if name in {"FI_DEV_NVLINK_BANDWIDTH_TOTAL", "gpu_mem_total"} or name.startswith("cpu_time_"):
        return False
    return name in _COUNTERS or name.startswith("vmstat_") or name.endswith("_total")
...
def describe_metric(name: str, endpoints: Sequence[str] = ()) -> dict[str, Any]:
    ...
    if n.startswith("trtllm_"):
        ...
        description = (
            "Observed process groups selected by the exporter; filter groupname and endpoint to inspect a process."
        )
        if "thread" in n:
    ...
    if n == "trtllm_iteration_latency_seconds":
        ...
        quality = (
            "Historical attached histogram sums/counts can cross label sets. Only recorded bucket deltas are used."
        )
        ...
        quality = "Historically exported as a gauge but incremented cumulatively for disconnect events; displayed as a counter rate."
