...
class Nodes:
    ...
    def from_slurm(
        ...
        Args:
            ...
            batch_host: When set, keep head and benchmark on this authoritative
                        Slurm batch host so power samples and windows share a
                        clock. Reserve dedicated infra from the tail instead.
        """
        dedicated_roles = [
            role
            for role, wanted in (
        ...
        if het_lists is not None:
            ...
            return cls._from_het_slurm(het_lists, etcd_nats_dedicated_node, batch_host)
        ...
        if batch_host is not None:
            if batch_host not in nodelist:
                raise ValueError(f"batch host {batch_host} is not in the SLURM allocation")
            if client_dedicated_node:
                raise ValueError("power telemetry cannot use a dedicated client away from the batch host")
            reserved: dict[str, str] = {}
            if frontend_dedicated_node:
                reserved["frontend"] = batch_host
            if etcd_nats_dedicated_node:
                if frontend_dedicated_node and colocate_dedicated_nodes:
                    reserved["infra"] = batch_host
                else:
                    candidates = [node for node in nodelist if node != batch_host]
                    if not candidates:
                        raise ValueError("dedicated infra requires a node other than the batch host")
                    reserved["infra"] = candidates[-1]
            worker = tuple(node for node in nodelist if node not in reserved.values())
            if not worker:
                raise ValueError("dedicated nodes must leave at least one worker node")
            return cls(head=batch_host, bench=batch_host, infra=reserved.get("infra", batch_host), worker=worker)
    ...
    def _from_het_slurm(
        ...
        if batch_host is not None and batch_host not in group0:
...
class RuntimeContext:
    ...
    def from_config(
        ...
        if config.telemetry.enabled:
            batch_host = os.environ.get("SLURMD_NODENAME")
            if not batch_host:
                raise ValueError("power telemetry requires SLURMD_NODENAME to identify the collector's batch host")
