    ...
    def _run_post_eval(self, stop_event: threading.Event) -> int:
        ...
        if os.environ.get("EVAL_ONLY", "false").lower() == "true":
            ...
            if not wait_for_model(
                ...
                return 1
        elif not wait_for_port(self.runtime.nodes.head, 8000, timeout=30):
        ...
        env_to_set.setdefault("MODEL_PATH", "/model")
        ...
        elif "EVAL_CONC" in env_to_set:
            logger.info("Eval concurrency (from recipe): %s", env_to_set["EVAL_CONC"])
        else:
