...
`SRTCTL_LOCAL_CONTAINER_IMAGE`, and either `dynamo.hash` or `dynamo.top_of_tree: true`. `dynamo.engine_mode: sidecar` runs stock SGLang plus `dynamo.sglang.sidecar` from the installed Dynamo checkout. The run creates
