...
- `dynamo.hash` or `dynamo.top_of_tree: true`. The first run builds a hash-keyed Dynamo wheel outside the container; later runs reuse it. With `dynamo.engine_mode: sidecar`, it runs the installed `dynamo.sglang.sidecar` module alongside stock SGLang.
