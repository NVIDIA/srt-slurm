...
| `numa_memory_bind` | bool \| None | `None` | Whether to prefix the trtllm worker command with `numactl -m 0,1`. None (default) enables it only for gb200/gb300/vrnvl72 prefill and decode workers (case-insensitive GPU type). True/False forces numactl on/off regardless of gpu_type or mode. |
