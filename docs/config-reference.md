...
Custom benchmarks with vLLM or SGLang sidecars automatically include the sidecar's `/metrics` endpoint on `DYN_SYSTEM_PORT` in `AIPERF_SERVER_METRICS_URLS`, alongside the native worker URLs. Each vLLM node-local DP pool has its own sidecar; multi-node SGLang endpoints have one on the leader. Native engine counters and Dynamo runtime counters remain separate endpoints. This does not change the worker URLs used for inference or profiling.

An explicit `AIPERF_SERVER_METRICS_URLS` in the recipe environment replaces automatic discovery. Include both the native metrics endpoints (or their relays) and each sidecar's system-port endpoint in that list. In particular, the native vLLM sidecar launch binds its HTTP server to loopback, so a benchmark on another node needs a reachable metrics relay; a relay exposing only native `/metrics` does not also expose Dynamo sidecar counters. Frontend collection is separate and must also be configured or discovered by the benchmark client.
