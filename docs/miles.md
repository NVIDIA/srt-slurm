...
- **Local mode has one filesystem per server, so episodes must not overlap.** The agent's commands run in the task's source directory, and `evaluate` stages the verifier at the fixed paths `/tests` and `/logs/verifier`, per container rather than per session. With 32 concurrent episodes on sa-b200 (job 15591) the loop ran end to end, 32 sessions, a GRPO step, but 31 evaluates failed on files another episode had removed. Run the server with `MAX_CONCURRENT_ENVS=1`; the Miles adapter queues episodes on the capacity signal, so a rollout is correct and slow rather than fast and meaningless. `TB2_WITHHOLD_TESTS=1` deletes `tests/` and `solution/` from the checkout on disk (the server keeps them in memory), so `git checkout -- .` the checkout between runs. Put `TB2_OUTPUT_DIR` under `/logs` so the per-episode terminal logs land with the job's artifacts. Real parallelism needs one filesystem per episode: per-episode enroot containers nested inside the trainer's container, which works on sa-b200 only with the host's enroot tooling at its own paths, or a hosted sandbox provider.
